...
function Client(options) {
  ...
  // allow driver to be overriden, defaults to pure javascript version
  if (options.driver) {
    pg = options.driver;
  }
...
Client.prototype.exec = function(query, values, cb) {
  ...
  var that = this;
  this.acquire(function(err, client, release) {
    if (err) return cb(err);
    that.execClient(client, query, values, function(err, result) {
      release();
      cb(err, result);
    });
  });
...
Client.prototype.series = function(statements, cb) {
  this._execSeriesParallel('eachSeries', statements, cb);
};


Client.prototype.parallel = function(statements, cb) {
  this._execSeriesParallel('each', statements, cb);
};
